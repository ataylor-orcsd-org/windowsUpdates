Import-module \\sb-lansweeper\DefaultPackageShare$\Scripts\PSWindowsUpdate
...
Function Reboot
{
    ...
    Write-Eventlog -Logname Application -Message "Rebooting" -Source AutomatedUpdates -id 99 -entrytype Warning -category 0
...
Clear-Host
if (!(test-path ` HKLM:\SYSTEM\CurrentControlSet\Services\Eventlog\Application\AutomatedUpdates )) {new-eventlog -Logname Application -source AutomatedUpdates -ErrorAction SilentlyContinue}

## Step 2 - Create a 'Job start' entry in your event log
$startTime = Get-date
$startLog = 'Automatic Updates started at ' +$startTime+ ' local time' 
Write-Eventlog -Logname Application -Message $startLog -Source AutomatedUpdates -id 1 -entrytype Warning -Category 0
lsClient
...
## Step 4 - Write errors during processing (typically part of a if statement)
    #Write-Eventlog -Logname Application -Message "" -Source AutomatedUpdates -id 100 -entrytype Warning -category 0

## Step 5 - Write end of process entry in your event log
$endTime = Get-date
$endLog = 'Automatic Updates ended at ' +$endTime+ ' local time'
Write-Eventlog -Logname Application -Message $endLog -Source AutomatedUpdates -id 9 -entrytype Warning -category 0
lsClient
...
Function UpdateLogs
{
	Get-WinEvent Application | Where {$_.ProviderName -Match "AutomatedUpdates"}
}
...
Function Get-WUInstall
{
	...
	Process
	{
        ...
        Write-Eventlog -Logname Application -Message "Connecting to $serviceName server. Please wait..." -Source AutomatedUpdates -id 2 -entrytype Warning -category 0
        lsClient
		Try
		{
			$search = ""
        ...
        Write-Eventlog -Logname Application -Message "Found [$PreFoundUpdatesToDownload] Updates in pre search criteria" -Source AutomatedUpdates -id 2 -entrytype Warning -category 0
        lsClient
        ...
        Write-Eventlog -Logname Application -Message "Found [$FoundUpdatesToDownload] Updates in post search criteria" -Source AutomatedUpdates -id 2 -entrytype Warning -category 0
        lsClient
		...
		If(!$ListOnly) 
		{
            ...
            Write-Eventlog -Logname Application -Message "Accept [$AcceptUpdatesToDownload] Updates to Download" -Source AutomatedUpdates -id 2 -entrytype Warning -category 0
            lsClient
            ...
            Write-Eventlog -Logname Application -Message "Downloaded [$ReadyUpdatesToInstall] Updates to Install" -Source AutomatedUpdates -id 2 -entrytype Warning -category 0
            lsClient
...
Function updateRegistryUpdateValues([string]$Name, [string]$value)
{
    $Key = "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate\Auto Update\Results\PSAU"
    If  ( -Not ( Test-Path "Registry::$Key")){New-Item -Path "Registry::$Key" -ItemType RegistryKey -Force}
    Set-ItemProperty -path "Registry::$Key" -Name $Name -Type "String" -Value $value
}

Function startupUpdateCheck([string]$Name, [string]$value)
{
...
Function updateRegistryAll
{
    if($PreFoundUpdatesToDownload -match "^[\d\.]+$")
        {
            $RegPreFoundUpdatesToDownload = $PreFoundUpdatesToDownload
        } else {
            $RegPreFoundUpdatesToDownload = 0
        }
    
    if($FoundUpdatesToDownload -match "^[\d\.]+$")
        {
            $RegFoundUpdatesToDownload = $FoundUpdatesToDownload
        } else {
            $RegFoundUpdatesToDownload = 0
        }
    
    if($AcceptUpdatesToDownload -match "^[\d\.]+$")
        {
            $RegAcceptUpdatesToDownload = $AcceptUpdatesToDownload
        } else {
            $RegAcceptUpdatesToDownload = 0
        }
    
    if($ReadyUpdatesToInstall -match "^[\d\.]+$")
        {
            $RegReadyUpdatesToInstall = $ReadyUpdatesToInstall
        } else {
            $RegReadyUpdatesToInstall = 0
        }
    
    
    
    updateRegistryUpdateValues "PreFoundUpdatesToDownload" $RegPreFoundUpdatesToDownload
    updateRegistryUpdateValues "FoundUpdatesToDownload" $RegFoundUpdatesToDownload
    updateRegistryUpdateValues "AcceptUpdatesToDownload" $RegAcceptUpdatesToDownload
    updateRegistryUpdateValues "ReadyUpdatesToInstall" $RegReadyUpdatesToInstall
}

Function lsPush
{
    [string]$pathToLSPush = "\\sb-lansweeper\DefaultPackageShare$\Scanners\lspush.exe"
    [string]$arguments = "helpdesk.orcsd.org"

    & $pathToLSPush $arguments
}

Function lsClient
{
    [string]$pathToLSClient = "\\sb-lansweeper\DefaultPackageShare$\Scanners\LSclient.exe"
    [string]$arguments = "helpdesk.orcsd.org"

    & $pathToLSClient $arguments
}
...
Report
lsClient
#startupUpdateCheck "AutomatedUpdates" "C:\WINDOWS\system32\WindowsPowerShell\v1.0\powershell.exe -noprofile -ExecutionPolicy bypass -command `"\\sb-lansweeper\DefaultPackageShare$\Scripts\AutoPowerShellUpdatesReportingOnly.ps1`""
Reboot